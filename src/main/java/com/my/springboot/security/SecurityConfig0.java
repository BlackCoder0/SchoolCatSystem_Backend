...
public class SecurityConfig0 extends WebSecurityConfigurerAdapter {
    ...
    public void configure(WebSecurity web) throws Exception {
        String[] AUTH_WHITELIST = {
                ...
                "/findpwd",
                // 静态资源
                // 因为 yml配置add-mappings: false 目的为了统一异常处理可以拦截到系统404和500异常 但是这个会把静态资源的配置失效
                // 所以要在此设置
                // 放行 图片存在磁盘的自定义目录中 url访问 如
                // http://localhost:8888/+yml配置中的static-path-pattern/+1.bmp
                // 如yml中 static-path-pattern: /static/
                // 则访问地址为http://localhost:8888/static/1.bmp此图片既可以放在工程的
                /// resources/static目录下 也可以放在自定义的磁盘路径下 如"file:"+windowsUploadPath,
                // "file:"+LinuxUploadPath 2个变量的值
